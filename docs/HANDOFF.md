...
## Starea curentă a proiectului (9 iul 2026 — Sprint 11 complet)
...
web/src/
  features/           ← logică organizată pe domeniu (feature-based)
    bookings/
      ...
      booking-form-dialog.tsx ← formular UNIFICAT (Sprint 11): mod compact (cameră+interval blocate, din calendar) vs complet (din „+ Adaugă"), aceeași stare/submit
      room-picker.tsx      ← UnitTypeSelect + RoomAllocation (tip + auto/manual), partajat compact/complet („schimbă camera")
      booking-date-fields.tsx ← CompactRoomHeader (mod compact) + DatesFields (mod complet), ambele cu hint minim nopți
      booking-price-field.tsx ← preț: total simplu editabil (implicit) ↔ link „editare avansată" → PriceOverrideEditor (3 moduri, neschimbat)
      booking-more-details.tsx ← secțiune expandabilă „Mai multe detalii": canal, note, cod promoțional
      booking-validation-panel.tsx ← erori blocante + warnings soft + toggle Manager Override, mereu vizibil
      booking-errors.ts    ← toastBookingError() — mapare unică mesaje eroare (sursă unică, era duplicată)
      booking-history.tsx ← istoric rezervare, afișează actorul (`{t("unit.event.by")} {actor_name}`, Sprint 11)
    ...
    guests/
      api.ts
      hooks.ts
      guest-combobox.tsx ← rămâne DOAR pentru re-asociere oaspete pe rezervare existentă (bookings/$bookingId.tsx)
      guest-quick-field.tsx ← câmp inline fără modal (Sprint 11): căutare + „Creează «text»" + navigare tastatură (săgeți/Enter), folosit acum peste tot la creare (înlocuiește guest-combobox)
      resolve-guest.ts     ← resolveGuestId() — creează oaspetele nou (findOrCreate) doar la submit, sursă unică
    properties/
      api.ts
      hooks.ts
  ...
  routes/             ← file-based TanStack Router
    ...
    _app/property/$propertyId/   ← URL scurt: DOAR propertyId (org dedus din property.org_id)
      ...
      calendar.tsx    ← fereastră glisantă 31 zile (Sprint 11, nu grilă pe lună calendaristică) + selecție tap-tap/drag pe celule libere → popover „Rezervare"/„Blocaj" precompletat
...
- **Combobox/typeahead cu selecție care nu pierde focusul → redeschide dropdown-ul explicit la tastare, nu doar pe `onFocus`** (Sprint 11, `guest-quick-field.tsx`). La selectarea unei opțiuni (click cu `onMouseDown` preventDefault, sau Enter din tastatură) input-ul rămâne focusat — nu se declanșează niciodată un blur/focus nou. Dacă redeschiderea dropdown-ului depinde doar de `onFocus`, userul poate tasta peste valoarea selectată fără ca dropdown-ul să reapară, până dă din întâmplare click în afară și înapoi. Fix: `onChange`-ul textului setează explicit `open=true`, nu se bazează pe evenimentul de focus.
- **Tabele de audit cu doar `actor_id uuid` (fără snapshot de email) se rezolvă în RPC-ul de citire prin `left join profiles + left join auth.users`, `coalesce(full_name, email)`** — pattern stabilit la Sprint 8.2 (`get_housekeeping_board`), reaplicat identic la Sprint 11 (`get_booking_events`, `booking_events.actor_id` era populat corect de la creare, dar pagina rezervării citea `select("*")` brut fără nicio rezolvare a actorului). Nu presupune că un RPC „similar" (aici `get_activity_feed`) acoperă automat toate suprafețele UI — verifică explicit fiecare loc care citește aceeași tabelă.

---

## Features implementate (rezumat)

- ✅ **Sprint 11 — Quick-create din calendar & unificarea formularului de rezervare**: obiectiv „mai ușor decât Excel" — rezervare/blocaj create direct din selecția pe calendar, fără să mai deschizi un formular gol. **Calendar**: grila pe lună calendaristică înlocuită cu o **fereastră glisantă de 31 zile** (tape-chart stil Beds24/Pynbooking) — săgețile mută fereastra cu ±7 zile, buton „Azi", label cross-month („iul. – aug. 2026"), toată poziționarea barelor pe indici de zi (`diffDays`, nu ziua-din-lună) ca rezervările să se vadă corect peste granița de lună. **Selecție de interval pe celule libere** ale unui rând: tap-tap (mobil+desktop) și drag cu mouse-ul (doar `pointerType==="mouse"`, touch rămâne liber pentru scroll orizontal); o hartă de obstacole per rând (rezervare/blocaj/turnover, closures excluse — sunt comerciale) oprește selecția la ultima zi liberă + `toast.error` cu motivul exact. Popover de acțiune la final → „Rezervare" sau „Blocaj", ambele precompletate cu camera+intervalul. **Formular de rezervare CONSOLIDAT**: `booking-form-dialog.tsx` era pe cale să se dubleze (un al doilea dialog „quick" cu aceeași logică de preț/validare/oaspete) — în loc de asta, un singur formular cu **mod compact** (context blocat din calendar: header cu cameră+interval, link „schimbă camera" dezvăluie același picker tip+auto/manual ca modul complet, cod partajat nu duplicat) și **mod complet** (din „+ Adaugă", fără preselectare). Progresive disclosure: preț implicit un total simplu editabil (detectează automat override-ul), cu link „editare avansată" spre editorul existent cu 3 moduri (total/ajustare/per-noapte, neschimbat); status ca toggle 2 butoane; canal+note+cod promo strânse sub „Mai multe detalii ▾"; validarea (erori/warnings/Manager Override) rămâne mereu vizibilă. **Oaspete**: `GuestQuickField` nou — un singur input fără modal, căutare live cu debounce, „Creează «text»" ca primă opțiune, navigare completă din tastatură (săgeți + Enter, prima opțiune mereu pre-evidențiată), creare efectivă abia la submit (`resolveGuestId`, dedupe server-side existent) — folosit acum peste tot la creare (`guest-combobox.tsx` rămâne doar pentru re-asociere pe rezervare existentă). **Actor pe istoricul rezervării**: `booking_events.actor_id` era populat corect de la creare dar nu era afișat nicăieri pe pagina rezervării — RPC nou `get_booking_events` (pattern identic Sprint 8.2: `left join profiles + auth.users`, `coalesce(full_name, email)`) + `booking-history.tsx` arată acum „de {nume/email}" ca restul istoricurilor din aplicație. Teste DB TEST 117 (**345 PASS**). Doc: [`docs/backend/rpc/bookings.md`](backend/rpc/bookings.md)

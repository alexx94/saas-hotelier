...
| **6.5 — Audit** *(implementat ca Sprint 7, scop revizuit)* | Tabel generic `entity_events` + trigger generic `app.audit_entity` — extinde audit-ul operațional existent (`unit_events`/`unit_type_events`/`booking_events`, Sprint 3) la `properties`/`guests`/`payments`/`rate_rules`/`promotions`/`stay_rules`/`arrival_rules`/`closures`, plus `get_activity_feed` (feed unificat per proprietate, filtre entity/event/dată). Gated pe `audit.view` (reutilizat, nicio permisiune nouă). **Nu** acoperă acțiunile admin cross-entitate inițial avute în plan (rol/acces/ownership/invitații — acelea rămân `organization_members`/`member_roles`/viitorul `organization_invites`, fără audit dedicat încă). | ✅ **done** (scop operațional) · ⏳ acțiuni admin | [rpc/audit.md](rpc/audit.md) |

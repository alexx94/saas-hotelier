...
Funcții interne (schema `app`, neapelabile din API): `app.create_booking_internal` ([bookings.md](bookings.md)), `app.find_or_create_guest_internal` ([guests.md](guests.md)), `app.sync_booking_payment` (trigger plăți, [payments.md](payments.md)), `app.compute_price` (motorul de preț, [pricing.md](pricing.md)), `app.resolve_stay` + `app.is_closed` (reguli de rezervare, [reservation-rules.md](reservation-rules.md)), `app.check_arrival_departure` (restricții sosire/plecare, [stay-restrictions.md](stay-restrictions.md)).
